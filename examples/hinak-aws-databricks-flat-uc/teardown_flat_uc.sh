...
terraform destroy \
  ...
  -var="prefix=$prefix" \
  -auto-approve || handle_error "Unity Catalog設定の削除に失敗しました。"

# ワークスペース削除
echo "Databricksワークスペースを削除しています..."
cd ../aws-databricks-flat || handle_error "aws-databricks-flat ディレクトリに移動できません。"
terraform init || handle_error "Terraform初期化に失敗しました。"
terraform destroy \
  -var="aws_profile=$aws_profile" \
  -var="region=$region" \
  -var="prefix=$prefix" \
  -auto-approve || handle_error "ワークスペース削除に失敗しました。"
